...
function cleanJsonString(str) {
  ...
  // Also check if it's an object containing an array (e.g. { "clips": [...] })
...
function normalizeClipsResult(parsed) {
  ...
  if (parsed && typeof parsed === 'object') {
    // Check common keys like clips, segments, data, result
...
function generateMockClips(transcript, videoDuration) {
  console.log('No working AI API Keys found. Generating fallback mock clips.');
  const clips = [];
  const clipLength = 25; // 25 seconds
  ...
  for (let i = 0; i < totalClips; i++) {
    ...
    clips.push({
      ...
      description: `Auto-generated clip focusing on the video segment from ${start} to ${end} seconds. This highlights details discussed during this segment.`,
      start,
      end,
    });
  }

  // If the video is extremely short and has no clips, add at least one clip
...
export async function identifyViralClips(transcript, videoDuration, settings = {}) {
  ...
  const mistralKey = settings.mistral_api_key || process.env.MISTRAL_API_KEY;
...
Respond ONLY with valid JSON.`;
  ...
  // Order providers based on active preference
  const providers = [
  ...
  // Move active provider to the front
  providers.sort((a, b) => (a.id === activeProvider ? -1 : b.id === activeProvider ? 1 : 0));

  for (const provider of providers) {
    if (provider.key) {
      try {
        console.log(`AI CLIP ANALYZER: Attempting with ${provider.name}...`);
  ...
  console.warn('All configured AI providers failed or no keys were provided. Falling back to mock clips.');
...
Respond ONLY with valid JSON.`;
  ...
  for (const provider of providers) {
    if (provider.key) {
      try {
        console.log(`HINGLISH TRANSLITERATOR: Attempting with ${provider.name}...`);
        const result = await provider.fn();
  ...
  console.warn('All transliteration AI attempts failed or no keys found. Returning original transcript.');
  return transcript;
}
