  ...
  for (const instance of invidiousInstances) {
    try {
      ...
      if (res.ok) {
        ...
        const mp4Stream = streams.find(s => s.container === 'mp4' || s.type?.includes('mp4') || s.qualityLabel?.includes('720')) || streams[0];
  ...
  for (const instance of pipedInstances) {
    try {
      ...
      if (res.ok) {
        ...
        const validStream = videoStreams.find(s => s.format === 'mp4' || s.mimeType?.includes('mp4') || s.quality === '720p') || videoStreams[0];
