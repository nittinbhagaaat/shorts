...
export async function downloadVideoClip(youtubeUrl, start, end, outputPath, customYtDlpPath, customFfmpegPath) {
  ...
  for (const bin of executablesToTry) {
    // Strategy 1: Standard yt-dlp section download with Android/MWeb extractor args
    try {
      const args = [
      ...
      if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 1000) {
        console.log(`[Video Downloader] Section download succeeded with ${bin}: ${outputPath}`);
        return;
      }
    } catch (err1) {
      console.warn(`[Video Downloader] Strategy 1 failed with ${bin}: ${err1.message}`);
      lastError = err1;
    }

    // Strategy 2: Fallback to best MP4 single stream with download sections
    try {
      const argsFallback = [
        '--ffmpeg-location', ffmpegDir,
        '--download-sections', `*${startSec}-${endSec}`,
    ...
    // Strategy 3: Stream URL extraction + direct FFmpeg slice
    try {
      const getUrlArgs = [
        '--extractor-args', 'youtube:player_client=android,web,mweb',
        '--no-check-certificates',
  ...
  throw new Error(`Failed to download video clip: ${lastError ? lastError.message : 'yt-dlp stream extraction failed.'}`);
}
