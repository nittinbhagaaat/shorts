...
export async function downloadVideoClip(youtubeUrl, start, end, outputPath, customYtDlpPath, customFfmpegPath) {
  ...
  const tryDownload = async (executable) => {
    try {
      await runCommandLine(executable, args);
      console.log(`Video clip successfully downloaded to: ${outputPath}`);
      return true;
    } catch (err) {
      console.warn(`yt-dlp failed with ${executable}. Trying fallback...`);
      return false;
    }
  };

  let success = await tryDownload(ytDlpExecutable);
  ...
  if (!success) {
    const fallbacks = [
      ...
      'yt-dlp',
      'python3 -m yt_dlp'
    ].filter((f) => f !== ytDlpExecutable);

    for (const fb of fallbacks) {
      success = await tryDownload(fb);
      if (success) break;
    }
  }

  if (!success) {
    throw new Error('Failed to download video clip with yt-dlp. Please verify yt-dlp is available.');
  }
}
