...
const DEFAULT_FFMPEG_PATH = '/opt/homebrew/opt/ffmpeg-full/bin/ffmpeg';
...
export async function downloadVideoClip(youtubeUrl, start, end, outputPath, customYtDlpPath, customFfmpegPath) {
  const ytDlpExecutable = (customYtDlpPath || process.env.YT_DLP_PATH || DEFAULT_YT_DLP_PATH).trim();
  const ffmpegExecutable = (customFfmpegPath || process.env.FFMPEG_PATH || DEFAULT_FFMPEG_PATH).trim();
  const ffmpegDir = path.dirname(ffmpegExecutable);
  
  // Format start and end as integers or floats
  ...
  try {
    ...
    // Fallback command: download using 'best' format which has pre-merged audio and video
    const fallbackArgs = [
      '--ffmpeg-location', ffmpegDir,
      '--download-sections', `*${startSec}-${endSec}`,
      '-f', 'best[ext=mp4]/best',
...
export function generateAssSubtitles(segments, clipStart, style = 'hormozi', isHorizontal = false) {
  ...
  // For horizontal aspect ratios, we scale font size down slightly so they are not massive
  const scale = isHorizontal ? 0.8 : 1.0;

  // Styles block for the ASS file
...
`;
  ...
  // Extract all words from segments and offset their timings relative to clipStart
  const words = [];
  segments.forEach(seg => {
    const rawWords = seg.text.split(/\s+/).filter(Boolean);
    if (rawWords.length === 0) return;

    // Linearly interpolate word timings inside the segment
    ...
    rawWords.forEach((word, idx) => {
      ...
      // Keep words within bounds (non-negative)
      words.push({
        text: word.replace(/[^\w\s\p{P}]/gu, ''), // clean word, keep punctuation
  ...
  if (style === 'hormozi') {
    // Alex Hormozi Style: Word-by-word animation inside 3-word phrase groups
    const phraseSize = 3;
    for (let i = 0; i < words.length; i += phraseSize) {
      const phraseWords = words.slice(i, i + phraseSize);
      if (phraseWords.length === 0) continue;

      // Create an event for each word within the phrase, highlighting the active word
      phraseWords.forEach((activeWord, activeIdx) => {
        const textParts = phraseWords.map((w, idx) => {
          const cleanText = w.text.toUpperCase();
          if (idx === activeIdx) {
            return `{\\c&H00FFFF&}${cleanText}`; // Yellow color tag
          } else {
            return `{\\c&HFFFFFF&}${cleanText}`; // White color tag
          }
        });
    ...
    // Minimalist & Classic: Group words into standard 5-6 word phrases, display them statically
    const phraseSize = 5;
    for (let i = 0; i < words.length; i += phraseSize) {
      const phraseWords = words.slice(i, i + phraseSize);
      if (phraseWords.length === 0) continue;
...
export async function renderFinalShort(inputPath, assPath, cropFocus, style, outputPath, isHorizontal = false, customFfmpegPath) {
  ...
  const relativeAssPath = assPath.replace(/\\/g, '/'); // normalize backslashes for windows/mac paths

  if (isHorizontal) {
    // Landscape 16:9 layout: no cropping, just burn in subtitles!
    videoFilter = `subtitles=${relativeAssPath}`;
  } else {
    // Portrait 9:16 layout: crop and burn in subtitles!
