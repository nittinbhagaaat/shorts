...
import { resolveDirectStreamUrl } from '@/lib/stream-resolver';

/**
 * Downloads a specific start-end segment of a YouTube video using direct stream resolution and yt-dlp fallbacks.
 */
export async function downloadVideoClip(youtubeUrl, start, end, outputPath, customYtDlpPath, customFfmpegPath, cookiesText) {
  ...
  if (cookiesText && cookiesText.trim()) {
    cookieFilePath = path.join(outDir, `cookies-${Date.now()}.txt`);
    try {
      fs.writeFileSync(cookieFilePath, cookiesText.trim(), 'utf8');
    } catch (e) {
      cookieFilePath = null;
    }
  }

  // STRATEGY 1 (Fastest & 100% Bot-Proof): Direct Stream Resolution via Cobalt/Invidious/Piped
  try {
    console.log(`[Video Downloader] Resolving direct stream URL for ${youtubeUrl}...`);
    const directStreamUrl = await resolveDirectStreamUrl(youtubeUrl);
    if (directStreamUrl) {
      console.log(`[Video Downloader] Slicing video segment directly with FFmpeg (${startSec}s to ${endSec}s)...`);
      const sliceArgs = [
        '-ss', `${startSec}`,
        '-to', `${endSec}`,
        '-i', directStreamUrl,
      ...
      if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 1000) {
        console.log(`[Video Downloader] Direct stream slice successfully created at: ${outputPath}`);
        if (cookieFilePath && fs.existsSync(cookieFilePath)) try { fs.unlinkSync(cookieFilePath); } catch (e) {}
        return;
      }
    }
  } catch (streamErr) {
    console.warn(`[Video Downloader] Direct stream resolution strategy failed:`, streamErr.message);
  }

  // STRATEGY 2: yt-dlp section download with cookies and client fallbacks
  ...
  for (const bin of executablesToTry) {
    try {
      ...
      if (cookieFilePath) {
        ...
        baseArgs.push(
          '--extractor-args', 'youtube:player_client=android_creator,android;player_skip=webpage,configs',
          '--user-agent', 'Mozilla/5.0 (Linux; Android 13; SM-G981B) AppleWebKit/537.36'
        );
      }

      baseArgs.push(youtubeUrl, '-o', outputPath);

      await runCommandLine(bin, baseArgs);
      if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 1000) {
        console.log(`[Video Downloader] yt-dlp download succeeded with ${bin}: ${outputPath}`);
        if (cookieFilePath && fs.existsSync(cookieFilePath)) try { fs.unlinkSync(cookieFilePath); } catch (e) {}
        return;
      }
    } catch (err1) {
      console.warn(`[Video Downloader] yt-dlp attempt failed with ${bin}:`, err1.message);
      lastError = err1;
    }
  }
  ...
  throw new Error(`Failed to download video clip: ${lastError ? lastError.message : 'Video stream unavailable.'}`);
}
