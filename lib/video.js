...
/**
 * Resolves local FFmpeg binary path across macOS, Linux, and Windows
 */
export function getFfmpegBinary(customPath) {
  ...
  for (const c of candidates) {
    try {
      if (c === 'ffmpeg' || fs.existsSync(/*turbopackIgnore: true*/ c)) return c;
    } catch (e) {}
  }
...
/**
 * Resolves local yt-dlp binary path across macOS, Linux, and Windows
 */
export function getYtDlpBinary(customPath) {
  if (customPath && customPath.trim()) return customPath.trim();
  ...
  for (const c of candidates) {
    try {
      if (c === 'yt-dlp' || c.startsWith('python3') || fs.existsSync(/*turbopackIgnore: true*/ c)) return c;
    } catch (e) {}
  }
...
function runCommandLine(command, args, timeoutMs = 90000) {
...
/**
 * Downloads a specific start-end segment of a YouTube video using local yt-dlp + ffmpeg
 */
export async function downloadVideoClip(youtubeUrl, start, end, outputPath, customYtDlpPath, customFfmpegPath) {
  ...
  for (const bin of executablesToTry) {
    try {
      ...
      if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 1000) {
        console.log(`[Video Downloader] Clip segment downloaded successfully with ${bin}: ${outputPath}`);
        return;
      }
    } catch (err) {
      console.warn(`[Video Downloader] Attempt with ${bin} failed:`, err.message);
      lastError = err;
    }
  }

  // Fallback: Best single MP4 format stream
  for (const bin of executablesToTry) {
    try {
      const fallbackArgs = [
        '--ffmpeg-location', ffmpegDir,
      ...
      await runCommandLine(bin, fallbackArgs);
      if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 1000) {
        console.log(`[Video Downloader] Fallback clip downloaded successfully: ${outputPath}`);
        return;
      }
    } catch (fallbackErr) {
  ...
  throw new Error(`Failed to download video clip: ${lastError ? lastError.message : 'Please check that yt-dlp and ffmpeg are installed locally.'}`);
}
...
/**
 * Crops a video to 9:16 vertical ratio and burns subtitles in using local FFmpeg.
 */
